...
`;
    ...
    const getItemsList = () => overlay.querySelector('#ldzItems');
    ...
    const updateScrollButtons = () => {
      ...
      const up = overlay.querySelector('#ldzScrollUp');
      const down = overlay.querySelector('#ldzScrollDown');
      if (!list || !up || !down) return;
      const maxScroll = Math.max(0, list.scrollHeight - list.clientHeight);
      const epsilon = 2;
      up.disabled = list.scrollTop <= epsilon;
      down.disabled = maxScroll <= epsilon || list.scrollTop >= (maxScroll - epsilon);
    };

    const scrollListBy = (multiplier) => {
      const list = getItemsList();
      if (!list) return;
      const delta = list.clientHeight * multiplier;
    ...
    const listForListeners = getItemsList();
    if (listForListeners) {
      listForListeners.addEventListener('scroll', updateScrollButtons);
    }
    if (scrollUpBtn) {
